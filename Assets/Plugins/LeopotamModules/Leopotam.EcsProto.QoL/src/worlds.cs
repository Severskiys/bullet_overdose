...
namespace Leopotam.EcsProto.QoL
{
    public abstract class ProtoAspectInject : IProtoAspect
    {
        ...
        public virtual void Init(ProtoWorld world)
        {
            ...
            foreach (var f in GetType().GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                ...
                if (_aspectType.IsAssignableFrom(f.FieldType))
                {
                    ...
                    {
                        throw new Exception(
                            $"аспект \"{f.FieldType.Name}\" должен иметь конструктор по умолчанию, либо экземпляр должен быть создан заранее");
                    }
#endif
                ...
                if (_poolType.IsAssignableFrom(f.FieldType))
                {
                    ...
                    {
                        throw new Exception(
                            $"пул \"{f.FieldType.Name}\" должен иметь конструктор по умолчанию, либо экземпляр должен быть создан заранее");
                    }
#endif
                ...
                if (_itType.IsAssignableFrom(f.FieldType))
                {
                    ...
                    if (it == null)
                    {
                        throw new Exception($"итератор \"{f.FieldType.Name}\" должен быть создан заранее");
                    }
#endif
                    _its ??= new(8);
                    _its.Add(it);
        ...
        public ProtoIt Iter()
        {
            ...
            if (_it == null)
            {
                throw new Exception("в аспекте нет пулов для итерирования по ним");
            }
#endif
            return _it;
        }
