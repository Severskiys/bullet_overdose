...
namespace Leopotam.EcsProto.Unity {
    ...
    public sealed class ProtoWorldDebugSystem : IProtoInitSystem, IProtoRunSystem, IProtoEventListener {
        ...
        public void Init (IProtoSystems systems) {
            ...
            for (int i = 0, iMax = gens.Len (); i < iMax; i++) {
                if (gens.Get (i) > 0) {
                    OnEntityCreated (ProtoEntity.FromIdx (i));
                }
            }
        }
            ...
            foreach (var pair in _dirtyEntities) {
                ...
                if (_world.EntityGen (entity) > 0) {
                    ...
                    for (int i = 0, iMax = _typesCache.Len (); i < iMax; i++) {
                        entityName = $"{entityName}:{EditorExtensions.GetCleanTypeName (_typesCache.Get (i))}";
                    }
                }
