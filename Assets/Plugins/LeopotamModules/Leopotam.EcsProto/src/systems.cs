...
namespace Leopotam.EcsProto
{
    ...
    public class ProtoSystems : IProtoSystems, IProtoBenchSystems
    {
        ...
        public IProtoSystems AddSystem(IProtoSystem system, string pointName = default)
        {
            ...
            {
                throw new Exception(
                    $"не могу добавить систему \"{system.GetType().Name}\", системы уже инициализированы");
            }
#endif
        ...
        public IProtoSystems AddService(object injectInstance, Type asType = default)
        {
            ...
            if (IsInited())
            {
                throw new Exception($"не могу добавить сервис с типом \"{type.Name}\", системы уже инициализированы");
            }

            if (injectInstance is IProtoSystem)
            {
                throw new Exception(
                    $"не могу добавить сервис с типом \"{type.Name}\", система не должна использоваться как сервис");
            }

            if (_services.ContainsKey(type))
            {
                throw new Exception($"не могу добавить сервис с типом \"{type.Name}\", такой тип уже существует");
            }
#endif
        ...
        public IProtoSystems AddModule(IProtoModule module)
        {
            ...
            {
                throw new Exception(
                    $"не могу добавить модуль \"{module.GetType().Name}\", системы уже инициализированы");
            }

            if (_defaultPointAdded)
            {
                throw new Exception(
                    $"не могу добавить модуль \"{module.GetType().Name}\", он должен быть зарегистрирован до первого вызова AddPoint()");
            }
#endif
            module.Init(this);
            return this;
        ...
        public IProtoSystems AddPoint(string pointName)
        {
            ...
            if (IsInited())
            {
                throw new Exception($"не могу добавить точку \"{pointName}\", системы уже инициализированы");
            }

            if (string.IsNullOrEmpty(pointName))
            {
                throw new Exception("не могу добавить точку без имени");
            }
#endif
            if (!_defaultPointAdded)
            {
        ...
        public IProtoSystems AddWorld(ProtoWorld world, string name)
        {
            ...
            if (IsInited())
            {
                throw new Exception($"не могу добавить мир с именем \"{name}\", системы уже инициализированы");
            }

            if (string.IsNullOrEmpty(name))
            {
                throw new Exception("не могу добавить мир с пустым именем");
            }

            if (_worldMap.ContainsKey(name))
            {
                throw new Exception($"не могу добавить мир с именем \"{name}\", имя уже существует");
            }
#endif
...
#if DEBUG
            if (!_worldMap.ContainsKey(worldName))
            {
                throw new Exception(
                    $"не могу найти мир с именем \"{worldName}\", его сперва надо зарегистрировать в системах");
            }
#endif
        ...
        public virtual void Init()
        {
            ...
            for (int i = 0, iMax = _allSystems.Len(); i < iMax; i++)
            {
                ...
                for (var ii = 0; ii < benchesItem.Length; ii++)
                {
                    benchesItem[ii] = -100;
                }
            ...
            for (int i = 0, iMax = _allSystems.Len(); i < iMax; i++)
            {
                if (_allSystems.Get(i) is IProtoInitSystem iSystem)
                {
                    ...
                    {
                        throw new Exception(
                            $"обнаружена пустая сущность в мире \"{worldName}\" после вызова {iSystem.GetType().Name}.Init()");
                    }
#endif
                }
            }
        ...
        public virtual void Run()
        {
            for (int i = 0, iMax = _runSystems.Len(); i < iMax; i++)
            {
...
#if DEBUG || LEOECSPROTO_SYSTEM_BENCHES
                _sw.Stop();
                _benches.Get(_runSystemIndices.Get(i))[(int)ProtoBenchType.Run] =
                    (int)(_sw.Elapsed.TotalMilliseconds * 100);
#endif
#if DEBUG
                var worldName = CheckForLeakedEntities(this);
                if (worldName != null)
                {
                    throw new Exception(
                        $"обнаружена пустая сущность в мире \"{worldName}\" после вызова {_runSystems.Get(i).GetType().Name}.Run()");
                }
#endif
            }
        }
        ...
        public virtual void Destroy()
        {
            for (int i = 0, iMax = _allSystems.Len(); i < iMax; i++)
            {
                if (_allSystems.Get(i) is IProtoDestroySystem dSystem)
                {
                    ...
                    {
                        throw new Exception(
                            $"обнаружена пустая сущность в мире \"{worldName}\" после вызова {dSystem.GetType().Name}.Destroy()");
                    }
#endif
                }
            }
        ...
        {
            if (ProtoWorld.CheckForLeakedEntities(systems.World()))
            {
                return "по умолчанию";
            }
