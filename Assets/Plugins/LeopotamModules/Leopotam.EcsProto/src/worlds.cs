...
namespace Leopotam.EcsProto
{
    ...
    public class ProtoWorld
    {
        ...
        public static bool CheckForLeakedEntities(ProtoWorld host)
        {
            if (host._leaked.Len() > 0)
            {
                for (int i = 0, iMax = host._leaked.Len(); i < iMax; i++)
                {
                    ...
                    if (host._entityGens.Get(e._id) > 0 &&
                        EntityMask.IsEmpty(host._entityMasks, host._entityMaskLen, e))
                    {
                        return true;
                    }
                }
        ...
        public ProtoWorld(IProtoAspect aspect, Config cfg = default)
        {
            ...
            if (_aspects.Count == 0)
            {
                throw new Exception("нет зарегистрированных аспектов");
            }
        ...
        public void Destroy()
        {
            ...
            if (_destroyed)
            {
                throw new Exception("мир уже не существует до вызова ProtoWorld.Destroy()");
            }

            if (CheckForLeakedEntities(this))
            {
                throw new Exception("обнаружена пустая сущность до вызова ProtoWorld.Destroy()");
            }
#endif
            _destroyed = true;
            ProtoEntity e;
        ...
        {
#if DEBUG
            if (!_aspects.ContainsKey(aType))
            {
                throw new Exception(
                    $"не могу получить аспект \"{aType.Name}\", его сперва надо зарегистрировать в мире");
            }
#endif
        ...
        public void AddAspect(IProtoAspect proto)
        {
            ...
            if (_inited)
            {
                throw new Exception($"не могу добавить аспект \"{proto.GetType().Name}\", мир уже инициализирован");
            }

            if (_aspects.ContainsKey(proto.GetType()))
            {
                throw new Exception($"не могу добавить аспект \"{proto.GetType().Name}\", он уже существует");
            }
#endif
        ...
        public void AddPool(IProtoPool pool)
        {
            ...
            if (_inited)
            {
                throw new Exception($"не могу добавить пул для компонента \"{cType.Name}\", мир уже инициализирован");
            }

            if (_poolsMap.ContainsKey(cType))
            {
                throw new Exception($"не могу добавить пул для компонента \"{cType.Name}\", он уже существует");
            }
#endif
        ...
        public IProtoPool Pool(Type cType)
        {
            ...
            {
                throw new Exception(
                    $"не могу получить пул для компонента \"{cType.Name}\", его сперва надо зарегистрировать в аспекте");
            }
#endif
